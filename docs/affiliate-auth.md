...
Magic links expire after 15 minutes and are single-use. Only HMAC-SHA-256 digests are stored in production, keyed with `AFFILIATE_SESSION_SECRET`. Affiliate sessions expire after 30 days and are revalidated against the affiliate’s active status on every private request.
...
delete from public.affiliate_admin_login_tokens
where (used_at is not null or expires_at < now())
  and created_at < now() - interval '7 days';

delete from public.affiliate_admin_sessions
where (revoked_at is not null or expires_at < now())
  and created_at < now() - interval '30 days';
```
