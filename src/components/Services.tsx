...
import { OptimizationFormModal } from '@/components/OptimizationFormModal';
...
export const Services = () => {
  const [selectedOffer, setSelectedOffer] = useState<ModalOffer | null>(null);
  ...
  return (
    <>
        ...
        <div className="container mx-auto px-4 md:px-6 relative z-10">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-7 max-w-5xl mx-auto">
            {content.services.map((service, index) => {
              ...
              const professional = service.price === '60 €';
              return (
                <motion.article key={service.title} initial={{ opacity: 0, y: 28 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * .1 }} className={cn('relative bg-white border rounded-3xl p-8 md:p-10 shadow-sm flex flex-col', service.popular ? 'border-brand-300 ring-4 ring-brand-50 shadow-xl shadow-brand-900/5' : 'border-slate-200')}>
                  ...
                  <button type="button" onClick={() => setSelectedOffer({
                    title: service.title,
                    offerLabel: professional ? 'Professionnel — 60 €' : 'Étudiant / Demandeur — 30 €',
                    amount: professional ? '60' : '30',
                    paypal: service.paypal,
                  })} className={cn('w-full rounded-xl px-6 py-4 font-bold text-center flex items-center justify-center gap-2 transition-all', service.popular ? 'bg-brand-600 text-white hover:bg-brand-700 shadow-lg shadow-brand-500/20' : 'border border-brand-200 text-brand-700 hover:bg-brand-50')}>
                    {service.cta}<ArrowUpRight size={18}/>
                  </button>
                  <p className="mt-4 text-xs text-slate-500 flex items-center justify-center gap-2"><ShieldCheck size={14}/>Demande enregistrée avant le paiement PayPal</p>
                </motion.article>
              );
            })}
          </div>
      ...
      <OptimizationFormModal offer={selectedOffer} onClose={() => setSelectedOffer(null)} />
    </>
  );
};
