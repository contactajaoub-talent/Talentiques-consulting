  ...
  return (
    <>
      <nav className={cn(
        ...
        <div className="w-full max-w-[1500px] mx-auto px-5 lg:px-8 flex items-center justify-between gap-8">
          ...
          <div className="hidden xl:flex items-center gap-6 xl:gap-8 2xl:gap-10 ml-auto">
            {navLinks.map((link) => (
              <Link key={link.name} href={link.href} className="text-sm xl:text-[15px] font-semibold text-slate-600 hover:text-brand-600 transition-colors whitespace-nowrap">
                {link.name}
              </Link>
            ))}
            <Link href="/#services" className="shrink-0 px-5 xl:px-6 py-2.5 rounded-full bg-brand-600 text-white text-sm xl:text-[15px] font-bold hover:bg-brand-700 hover:shadow-lg transition-all flex items-center gap-2 whitespace-nowrap">
              Choisir mon offre <ArrowRight size={16} />
            </Link>
          </div>

          <button className="xl:hidden p-2 text-slate-800 relative z-50" onClick={() => setMobileMenuOpen(!mobileMenuOpen)} aria-label="Menu">
        ...
        <AnimatePresence>
          {mobileMenuOpen && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="xl:hidden fixed inset-0 top-0 bg-white z-40 flex flex-col items-center justify-center pt-20">
